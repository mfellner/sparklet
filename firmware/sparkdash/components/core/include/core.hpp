...
struct Value { double value=0; bool valid=false; };
...
struct Node {
 ...
 Role role=Role::Standalone; Online online=Online::Unknown;
 ...
 uint64_t received_ms=0; bool received=false;
};
struct Cache { std::array<Node,MaxNodes> nodes{}; size_t count=0, selected=0; bool overflow=false; uint32_t revision=0; };
struct Url { char host[128]{}, prefix[192]{}; uint16_t port=80; };
struct Connection { uint32_t version=1; char ssid[33]{}, password[65]{}, url[320]="http://dgx01.local:5555"; };
struct Preferences { uint32_t version=1; uint8_t brightness=60; uint16_t dim_seconds=120; };
...
struct Scheduler {
 ...
 struct Work { Kind kind=Kind::None; size_t index=0; };
 uint64_t list_due=0, foreground_due=0, background_due=0, retry_due=0;
 size_t round_robin=0; unsigned failures=0, retry_step=0; bool list_required=true, urgent=false;
 Work next(uint64_t now,size_t count,size_t selected) const;
 void completed(Work,uint64_t now,size_t count);
 void select(){ urgent=true; foreground_due=0; }
 void reconnect(){list_required=true; retry_due=0; failures=retry_step=0;}
 void failed(uint64_t now,uint32_t jitter=0);
 void success(){failures=retry_step=0; retry_due=0;}
};
}
