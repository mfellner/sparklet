...
def main():
    ...
    parser.add_argument('--output', type=Path, required=True, help='new ignored result JSON')
    ...
    ports = [p for p in list_ports.comports() if (p.vid, p.pid, p.serial_number) ==
             (0x303a, 0x1001, 'D4:05:92:B9:04:28')]
    if len(ports) != 1:
        ...
        with result_path.with_suffix('.raw').open('xb') as raw:
            while time.monotonic() - started < args.seconds:
                ...
                while b'\n' in buffer:
                    ...
                    if any(marker in line for marker in ('Guru Meditation', 'assert failed', 'abort() was called')):
                    ...
                    if 'diagnostics:' in line:
                        ...
                        fields['host_elapsed_ms'] = round((time.monotonic() - started) * 1000)
    ...
    for key, limit in [('heap', 32768), ('largest', 16384), ('stack', 1024), ('net_stack', 1024), ('ui_stack', 1024)]:
        if live and min(s.get(key, 0) for s in live) < limit:
            failures.append(f'Observed {key} below {limit}')
    result = {'duration_seconds': args.seconds, 'samples': samples, 'failures': failures,
              'scope': 'USB counters and sampled idle memory only; not touch timing or maximum response load'}
    result_path.write_text(json.dumps(result, indent=2) + '\n')
    print(json.dumps({'samples': len(samples), 'live_samples': len(live), 'failures': failures,
