...
## Reproducibility and contract coverage

Enabled ESP-IDF's APP_REPRODUCIBLE_BUILD option, removing date/time and path variability. Normal and separate `build-repro` builds produced identical application, bootloader and partition binaries. Application SHA-256: `59162a6aad6c06c3fc5a9bb68dbfed910e0c1cfd8ac48c7bf76e34c19eca86e1` (1,753,280 bytes); bootloader SHA-256: `9ea3702d525fe8f579cdc02445d3b663b95786db64ed73e2228a3d899e09f967`; partition-table SHA-256: `b16b1b27ccfeecedd026631c54e77065d3913bca810cad5cfe77525fc9938f75`. Evidence: ignored `logs/reproducible-binaries.json`.

Expanded shared host tests for unknown/standalone roles, worker compatibility and relationships, reported-zero memory fallback, enabled-network fallback, UTF-8 parser label limits, ID length/overflow duplicates, node addition/removal, and metadata refresh. This caught an omitted `kind` copy during list reconciliation; fixed it. Sanitizer tests passed. The packager now supplies a standalone README, source README, USB enumeration helper, recovery instructions, and a ZIP with external checksum.


## Extracted release bundle check
